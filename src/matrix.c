                ...
                for (int BLOCK_COL = col; BLOCK_COL < col + BLOCK_SIZE; BLOCK_COL++) {
                    ...
                    if (BLOCK_ROW < mat1->rows && BLOCK_COL < mat2->cols) {
                        ...
                        for (int i = 0; i < (mat1->cols / 16) * 16; i+= 16) {
                            ...
                            // Zeroth 4 
                            mat1_input = _mm256_loadu_pd((mat1->data + BLOCK_ROW * mat1->cols + i));
                            mat2T_input = _mm256_loadu_pd((transpose->data + BLOCK_COL * transpose->cols + i ));
                            partial_row_sum = _mm256_fmadd_pd(mat1_input, mat2T_input, partial_row_sum);
                            // First 4 
                            mat1_input = _mm256_loadu_pd((mat1->data + BLOCK_ROW * mat1->cols + i + 4));
                            mat2T_input = _mm256_loadu_pd((transpose->data + BLOCK_COL * transpose->cols + i + 4));
                            partial_row_sum = _mm256_fmadd_pd(mat1_input, mat2T_input, partial_row_sum);
                            // Second 4 
                            mat1_input = _mm256_loadu_pd((mat1->data + BLOCK_ROW * mat1->cols + i + 8));
                            mat2T_input = _mm256_loadu_pd((transpose->data + BLOCK_COL * transpose->cols + i + 8));
                            partial_row_sum = _mm256_fmadd_pd(mat1_input, mat2T_input, partial_row_sum);
                            // Third 4 
                            mat1_input = _mm256_loadu_pd((mat1->data + BLOCK_ROW * mat1->cols + i + 12));
                            mat2T_input = _mm256_loadu_pd((transpose->data + BLOCK_COL * transpose->cols + i + 12));
                            partial_row_sum = _mm256_fmadd_pd(mat1_input, mat2T_input, partial_row_sum);
                        }
                        double temp[4];
